...
class TestCLI:
    ...
    def __init__(self):
        ...
        self.system_prompt = (
            ...
            "- tools/doordash/ — order food on DoorDash (OWNER ONLY): search, "
            "find_nearby_stores, store_details, menu, find_items, item_details, cart, "
            "order, promo, address, payment_methods\n"
            ...
            "The flow is search.py (restaurants) or find_nearby_stores.py "
            "(grocery/alcohol/convenience/pets/retail — that's the entry point for "
            "anything that isn't a restaurant) -> menu.py or find_items.py -> "
            "cart.py add -> order.py preview -> order.py place --confirm.\n"
            "'same as last time' / 'order that again' -> order.py reorder --order-uuid "
            "(from order.py history). It rebuilds the cart WITH all the modifiers and "
            "charges nothing; feed its cart_uuid to order.py preview. Never rebuild a "
            "past order by hand from history — history drops modifiers.\n"
            "Before previewing, promo.py list --store-id can check for deals at that "
            "store; promo.py apply puts one on the cart (promo.py remove undoes it).\n"
            "Wrong thing in the cart? cart.py show to get the line id, then cart.py "
            "remove --cart-item-id — don't clear the cart and start over.\n"
            "store_details.py is the only way to get a store's address, so use it to "
            "tell chain locations apart.\n"
            "Flags passed to order.py preview (--scheduled-time, --fulfillment, "
            "--priority, --no-apply-credits) MUST be repeated identically on "
            "order.py place, or the charge won't match the quote.\n"
            ...
            "Everything else is safe — just run it: search, find_nearby_stores, "
            "store_details, menu, find_items, item_details, cart show/add/remove/clear, "
            "order preview/reorder/checkout-url/history/receipt/status, promo "
            "list/apply/remove, address, payment_methods. reorder makes a cart, it "
            "does not buy anything; promo apply and cart remove are both reversible.\n"
